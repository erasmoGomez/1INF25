...
void Inventario::asignar_memoria(char* tipo){
...
int Inventario::buscar_item(const char*nombre){
    char  nombre_item[30];
    for(int i = 0; i<n_items; i++){
        items[i]->GetNombre(nombre_item); //Seria virtual o no? no seria virtual xq necesito el del padre.
        if(strcmp(nombre_item, nombre)==0){
            items[i]->SetEquipado(true);
...
void Inventario::mostrar_items(ostream& output) const {
    cout<<"Mostrar Todos los Items"<<endl;
    for(int i=0; i< n_items; i++)
