...
void *leer_registro(ifstream &input) {
    ...
    ptr_codigo = new int;
    *ptr_codigo = codigo; // ptr_codigo[0] = codigo / ptr_codigo = &codigo;
    input.get();
...
void cargar_alumnos(void *&alumnos,
    ...
    void **arreglo_registros, *registro;
    arreglo_registros = nullptr;
    while (true) {
        registro = leer_registro(input);
        if (input.eof()) break;
        if (cantidad_alumnos == capacidad)
            incrementar_memoria(arreglo_registros, cantidad_alumnos, capacidad);
        arreglo_registros[cantidad_alumnos - 1] = registro;
        cantidad_alumnos++;
    }
    alumnos = arreglo_registros;
}
