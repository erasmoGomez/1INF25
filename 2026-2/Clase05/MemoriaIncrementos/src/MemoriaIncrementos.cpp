...
void incrementar_espacios(int *&codigos, int &n_datos, int &capacidad){
    int *aux;
    capacidad += INCREMENTO; //Incrementar la capacidad

    if(codigos == nullptr) //Esto solo pasa al inicio
        codigos = new int[capacidad]{}; //Importante {}
    else {
        //codigos: llega con 5 espacios
        //Aprovechamos de una reserva dinamica AUXILIAR
        aux = new int[capacidad]{}; // capacidad seria 10
        //aux: se reservan 15 espacios
        for(int i = 0; i<n_datos; i++)
            aux[i] = codigos[i];
        //aux: tiene los primeros 5 valores actualizados desde codigos;
...
void recorte(int *&codigos, int n_datos) {
    int* aux;
    aux = new int[n_datos]{};
    for(int i = 0; i<n_datos; i++) {
        aux[i] = codigos[i];
...
void leer_datos(int* &codigos, int &n_datos){
    ...
    while(true){
        ...
        if(n_datos == capacidad)
            incrementar_espacios(codigos, n_datos, capacidad);
